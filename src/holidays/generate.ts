...
export default () => {
   ...
   * 根据国务院办公厅发布的2025年部分节假日安排通知，全体公民放假假日增加2天，其中春节、劳动节各增加1天。春节自农历除夕起放假调休8天，国庆节自10月1日起放假调休7天（如逢中秋节则合并放假8天），劳动节放假调休5天，元旦、清明节、端午节、中秋节分别放假调休或连休3天（如逢周三则只在当日放假）。具体安排如下：
   * 一、元旦：1月1日放假1天，不调休。
  ...
  arrangement.y(2025)
    .ny().r(1, 1)
    .s().r(1, 28).to(2, 4).w(1, 26).w(2, 8)
    .t().r(4, 4).to(4, 6)
    .l().r(5, 1).to(5, 5).w(4, 27)
    .d().r(5, 31).to(6, 2)
    .n().r(10, 1).to(10, 8).w(9, 28).w(10, 11)
    .m().r(10, 3) // 中秋节实际为10月3日（周四），与国庆合并放假
